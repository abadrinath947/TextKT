...
def encode(ques, ans):
    ...
    last_idx = (input_ids == 0).int().argmax(dim = -1)
    idx = 0
    all_feat = []
    while idx < input_ids.shape[0]:
        ...
        all_feat.append(out['encoder_last_hidden_state'].detach().cpu()[torch.arange(len(out['encoder_last_hidden_state'])), 
            last_idx[idx:idx+encoder_batch_size]].numpy())
