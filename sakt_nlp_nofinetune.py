...
def encode(ques, ans):
    with torch.no_grad():
        input_ids = tokenizer(ques.to_list(), return_tensors="pt", padding=True, truncation=True).input_ids
        labels = tokenizer(ans.to_list(), return_tensors="pt", padding=True, truncation=True).input_ids
        idx = 0
        all_feat = []
        while idx < input_ids.shape[0]:
            ...
            all_feat.append(out['encoder_last_hidden_state'][:, -1].detach().cpu().numpy())
            del out
...
def preprocess_data(data):
    ...
    features = ['skill_idx', 'correct'] + ohe_column_names + [i for i in range(512)] #+ ['response_time', 'attempt_count', 'hint_count', 'first_action', 'position']
...
if __name__ == '__main__': 
    ...
    tokenizer = T5Tokenizer.from_pretrained("t5-tiny")
    encoder = T5ForConditionalGeneration.from_pretrained("t5-tiny").cuda()

    config = GPTConfig(vocab_size = 1 * len(ohe.get_feature_names_out()), block_size = block_size, n_layer = 2, n_head = 16, n_embd = 256, input_size = 640, bkt = False)
