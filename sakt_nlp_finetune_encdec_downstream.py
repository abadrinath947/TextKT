...
def encode(ques_ans):
    ques, ans = ques_ans[..., 0], ques_ans[..., 1]
    input_ids = tokenizer(ques.ravel().tolist(), return_tensors="pt", padding=True, truncation=True).input_ids
    labels = tokenizer(ans.ravel().tolist(), return_tensors="pt", padding=True, truncation=True).input_ids
    labels[labels == tokenizer.pad_token_id] = -100
    last_dec_idx = (labels == -100).int().argmax(dim = -1)
    last_enc_idx = (input_ids == 0).int().argmax(dim = -1)
    idx = 0
    all_feat = []
    loss = 0
    while idx < input_ids.shape[0]:
        ...
        enc, dec = out['encoder_last_hidden_state'], out['decoder_hidden_states']
        all_feat.append(
            torch.cat([
                dec[torch.arange(len(dec)), last_dec_idx[idx:idx+encoder_batch_size]],
                enc[torch.arange(len(dec)), last_enc_idx[idx:idx+encoder_batch_size]].to(dec.device)     
            ], dim = -1)
        )
...
def preprocess_data(data):
    ...
    features = ['skill_idx', 'correct'] + ohe_column_names + ['qtxt', 'atxt'] # + [i for i in range(512)] #+ ['response_time', 'attempt_count', 'hint_count', 'first_action', 'position']
...
def construct_batches(raw_data, epoch = 0, val = False):
    ...
    for _ in range(len(user_ids) // batch_size):
        ...
        X = torch.tensor(batch[:, :-1, ..., 1:-2].astype(float), requires_grad=True, dtype=torch.float32).cuda()
...
if __name__ == '__main__': 
    ...
    config = GPTConfig(vocab_size = 1 * len(ohe.get_feature_names_out()), block_size = block_size, n_layer = 4, n_head = 16, n_embd = 256, input_size = 128 + 512 + 512, bkt = False)
