...
if __name__ == '__main__': 
    ...
    config = GPTConfig(vocab_size = 1 * len(ohe.get_feature_names_out()), block_size = block_size, n_layer = 5, n_head = 16, n_embd = 256, input_size = 896, bkt = False)
    model = GPT(config).cuda()
    print("Total Parameters:", sum(p.numel() for p in model.parameters()))
    optimizer = optim.AdamW([
            {"params": encoder.decoder.block[-1].parameters(), "lr": 3e-5},
            {"params": encoder.encoder.block[-1].parameters(), "lr": 3e-5},
