...
if __name__ == '__main__': 
    ...
    config = GPTConfig(vocab_size = 1 * len(ohe.get_feature_names_out()), block_size = block_size, n_layer = 6, n_head = 16, n_embd = 256, input_size = 640, bkt = False)
    model = GPT(config).cuda()
    print("Total Parameters:", sum(p.numel() for p in model.parameters()))
    optimizer = optim.AdamW(itertools.chain(model.parameters(), encoder.parameters(), 
                                           ), lr = 3e-4)
    def train(num_epochs):
        for epoch in range(num_epochs):
            ...
            for X, y, ques_ans in pbar:
                ...
                mask = (y[..., -1] != -1000).ravel()
                loss = F.binary_cross_entropy(output[mask], y[..., -1:].ravel()[mask]) + 0.1 * loss_encoder
                loss.backward()
                optimizer.step()
